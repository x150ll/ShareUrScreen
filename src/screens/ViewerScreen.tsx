...
// Hook مخصص — يمنع إطفاء الشاشة فقط أثناء المشاهدة الفعلية
function useConditionalKeepAwake(active: boolean) {
  useKeepAwake(active ? 'viewer-active' : undefined);
}
...
export default function ViewerScreen({ navigation }: any) {
  ...
  const join = async () => {
    ...
    try {
      ...
      await viewerSvc.current.connect(
        ...
        async () => {
          clearTimeout(connectTimeout);
          // فحص هل الجلسة انتهت أم انقطع الاتصال فقط
          try {
            const session = await SignalingService.getSession(clean);
            setDisconnectReason(
  ...
  const tryReconnect = async () => {
    ...
    try {
      await viewerSvc.current.disconnect().catch(() => {});
      // تحقق أولاً هل الجلسة لا تزال موجودة
  ...
  return (
    <View style={s.videoWrap}>
      ...
      {(!fullscreen || showControls) && (
        <Animated.View
          style={[s.controls, fullscreen && { opacity: controlsAnim }]}
          pointerEvents={fullscreen && !showControls ? 'none' : 'auto'}
        >
          {/* شريط علوي */}
